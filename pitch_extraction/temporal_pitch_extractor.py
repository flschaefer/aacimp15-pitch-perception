...
class TemporalPitchExtractor(object):
    ...
    def extract(self, spikes, sample_rate):
        ...
        lmda = 40000
        lower_boundary = 0
        if number_of_points_in_channel < 24000:
            upper_boundary = number_of_points_in_channel
        else:
        ...
        number_of_iterations = len(
            np.r_[lower_boundary:upper_boundary:step_tau])  # (upper_boundary-lower_boundary)/step_tau
        ...
        for i in range(number_of_iterations):
            for j in range(number_of_channels):
                iteration[i] += self.integration(lower_boundary, upper_boundary, sample_rate, dt, channels_data, j,
                                                 i * step_tau, lmda)
    ...
    def acor(self, channels_data, channel_index, tau, t, t_current, lmda):
        """
    ...
    def integration(self, lower_boundary, upper_boundary, sample_rate, dt, channels_data, channel_index, tau, lmda):
        """
        trapezoidal integration of acor
        """
        result = self.acor(channels_data, channel_index, tau, upper_boundary, 0 + tau, lmda) * dt / 2.0
        result += self.acor(channels_data, channel_index, tau, upper_boundary, upper_boundary + tau, lmda) * dt / 2.0
        for i in range(1, int(round((upper_boundary - lower_boundary) / (sample_rate * dt * 1.0)) - 1)):
            result += self.acor(channels_data, channel_index, tau, upper_boundary, i + tau, lmda) * dt * 1.0
        return result
